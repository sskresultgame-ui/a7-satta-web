...
import { getGameChartFromMongo } from "@/lib/mongodb";
import type { GameChartData } from "@/lib/types";
import { memGet, memSet, CHART_CACHE_HEADERS } from "@/lib/api-helpers";

// Homepage uses Hinglish display spellings. Normalize before any lookup so the
...
export async function GET(req: NextRequest) {
  ...
  const fresh = await getGameChartFromMongo(slug, month, year);
  if (fresh) {
    memSet(cacheKey, fresh, 300);
